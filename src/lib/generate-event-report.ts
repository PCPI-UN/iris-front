...
function lightGreyFill(): ExcelJS.Fill {
  return { type: "pattern", pattern: "solid", fgColor: { argb: COLORS.light2 } };
}
...
function buildDashboardSheet(
  ...
  // ── Row 3: Fecha de creación del reporte ─────────────────────────────────
  ws.mergeCells("A3:F3");
  ws.mergeCells("G3:L3");
  ...
  const summaryRows: [string, number][] = [
    ...
    ["Total de proyectos enviados", dash.submittedProjects],
    ["Aprovados", dash.approved],
  ...
  summaryRows.forEach(([label, value], i) => {
    ...
    const fill = isAlternate
      ? ({ type: "pattern", pattern: "solid", fgColor: { argb: COLORS.transparent } } as ExcelJS.Fill)
      : lightGreyFill();
  ...
  // ── Row 19: PROYECTOS POR CATEGORÍA ───────────────────────────────────────
  ...
  missingHeader.value = "MISSING DOCUMENTS";
  ...
  dash.missingDocuments.forEach(({ projectName, missing }, i) => {
...
function buildProjectsSummarySheet(wb: ExcelJS.Workbook, projects: Project[]) {
  ...
  const headers = [
    ...
    { label: "Team leader", align: "center" },
  ...
  projects.forEach((p, i) => {
    ...
    values.forEach((val, ci) => {
      ...
      cell.border = thinBorder(["left", "right", "top", "bottom"]);
      if (ci === 9) cell.numFmt = "dd/mm/yyyy";
    });
  });
}
...
function buildJudgesSheet(wb: ExcelJS.Workbook, projects: Project[]) {
  ...
  const headers = [
    ...
    { label: "Fecha de asignación", align: "center" },
  ...
  let globalIdx = 0;
  for (const p of projects) {
    for (const judge of p.assignedJudges) {
      const row = ws.getRow(rowNum);
      row.height = 19.95;
